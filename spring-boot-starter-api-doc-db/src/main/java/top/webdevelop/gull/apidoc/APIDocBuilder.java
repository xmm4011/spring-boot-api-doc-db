...
public class APIDocBuilder {
    ...
    public APIDoc build() {
        APIDoc apiDoc = new APIDoc(parseAPIDocUrl(), parseAPIDocAction());
        apiDoc.setRequest(parseAPIDocRequest());
        apiDoc.setResponse(parseAPIDocResponse());
        return apiDoc;
    }
    ...
    private List<APIDocField> parseAPIDocRequest() {
    ...
    private List<APIDocField> parseAPIDocResponse() {
        Class<?> returnType = this.method.getReturnType();
        if (returnType.equals(ModelAndView.class)) {
            return Collections.emptyList();
        }
        if (ClassUtils.isSingleFieldType(returnType)) {
            return Collections.singletonList(new APIDocField("direct return", APIDocFieldType.parse(returnType), false));
        }
    ...
    private List<APIDocField> parseAPIDocFieldByParameter(Type parentType, MethodParameter parameter, Class<?> clazz) {
        if (ClassUtils.isSingleFieldType(clazz)) {
            RequestParam requestParam = parameter.getParameterAnnotation(RequestParam.class);
            APIDocField field = new APIDocField((requestParam == null || StringUtils.isEmpty(requestParam.name())) ? parameter.getParameterName() : requestParam.name(),
        ...
        List<APIDocField> fields = parseAPIDocFieldByClass(parentType, beanClasses, clazz.getSuperclass(), clazz, parentType);
        if (ClassUtils.isListType(clazz)) {
            return Collections.singletonList(new APIDocField("direct parameter", APIDocFieldType.parse(clazz), false, fields));
        }
        return fields;
    }

    private List<APIDocField> parseAPIDocFieldByClass(Type parentType, List<Class<?>> beanClasses, Class<?> parent, Class<?> clazz, Type type) {
        if (ClassUtils.isListType(clazz)) {
    ...
    private List<APIDocField> parseAPIDocFieldByList(Type parentType, List<Class<?>> beanClasses, Class<?> parent, Class<?> clazz, Type type) {
        if (clazz.isArray()) {
            ...
            if (ClassUtils.isSingleFieldType(rawType)) {
                return Collections.singletonList(new APIDocField("direct parameter", APIDocFieldType.parse(rawType), false));
            }
    ...
    private List<APIDocField> parseAPIDocFieldByBean(Type parentType, List<Class<?>> beanClasses, Class<?> parent, Class<?> clazz, Type type) {
        List<APIDocField> fields = new ArrayList<>();
        ...
        if (ClassUtils.isMapType(clazz)) {
            ...
            Class<?> rawType = getRawType(actualTypeArgument);
            fields.add(new APIDocField("dynamic key", APIDocFieldType.parse(rawType), false,
                    parseAPIDocFieldByClass(parentType, beanClasses, parent, rawType, actualTypeArgument)));
            return fields;
        }
    ...
    private APIDocField parseAPIDocFieldByPropertyDescriptor(Type parentType, List<Class<?>> beanClasses, Class<?> clazz, PropertyDescriptor propertyDescriptor) {
        ...
        APIDocField field = new APIDocField(propertyDescriptor.getName(),
                APIDocFieldType.parse(propertyDescriptor.getPropertyType()),
                hasRequiredAnnotation(clazz, propertyDescriptor),
                parseAPIDocFieldByClass(parentType, beanClasses, clazz, propertyDescriptor.getPropertyType(), propertyDescriptor.getReadMethod().getGenericReturnType()));
